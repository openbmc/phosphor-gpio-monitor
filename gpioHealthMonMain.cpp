...
using PropertyType = std::variant<std::string, std::vector<std::string>>;
...
class GetStatus : GetStatus_inherit
{
  public:
    explicit GetStatus(sdbusplus::bus_t& bus, auto path, bool healthy,
        ...
        GetStatus_inherit(bus, path), entityObjPath(entityObjPath)
    {
                        ...
                        const std::string& objPath)
{
    ...
    if (properties.find("HealthCriteriaID") == properties.end())
    {
    ...
    {
        /* Find the GPIO line */
        lineName = std::get<std::string>(properties["HealthCriteriaID"]);
        lineMsg += lineName;
    ...
    gpios.push_back(std::make_unique<phosphor::gpio::GpioMonitor>(
        ...
        healthyPolarity, currentHealthy));
    dbusObjects.push_back(std::make_unique<GetStatus>(
        conn, gpioObjectPath.c_str(), currentHealthy, objPath));
    return true;
}
