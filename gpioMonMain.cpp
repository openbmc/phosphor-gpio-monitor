...
int main(int argc, char** argv)
{
    ...
    // Parse out file name of config json file.
    std::string gpioFileName = (options)["config"];
    if (gpioFileName == phosphor::gpio::ArgumentParser::emptyString)
    {
        log<level::ERR>("GPIO monitor config file not specified");
        gpioMonUsage(argv);
    }
    ...
    for (auto& obj : gpioMonObj)
    {
        ...
        if (line == NULL)
        {
            log<level::ERR>("Failed to find the gpio line");
            return -1;
        }
        else
        {
            log<level::INFO>("GPIO line monitor started");
        }
        ...
        if (obj.find("EventMon") != obj.end())
        {
            ...
            {
                log<level::ERR>("Incorrect GPIO monitor event defined");
                return -1;
            }
